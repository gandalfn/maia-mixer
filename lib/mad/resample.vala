...
public class MaiaMixer.Mad.Resample : MaiaMixer.Filters.Resample
{
    // properties
    ...
    {
        if (sample_rate == 0 || inSample.sample_rate == 0 || sample_rate == inSample.sample_rate)
        {
            return 0;
        }

        // Calculate ratio
        m_Ratio = global::Mad.Fixed ((double)inSample.sample_rate / (double)GLib.Math.fabs (sample_rate));
        if (m_Ratio == 0)
        {
            return 0;
        }
    ...
    process (Audio.Sample inSample)
    {
        ...
        if (inSample.sample_rate != sample_rate)
        {
            ...
            if (length > 0)
            {
                ret = new Audio.Sample (inSample.channels, length, (uint)GLib.Math.fabs (sample_rate));
